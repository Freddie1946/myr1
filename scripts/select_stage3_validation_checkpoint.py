...
def validate_candidate(metrics_path: Path, *, expected_step: int, expected_model: Path) -> dict[str, Any]:
    try:
    ...
    predictions = Path(metrics.get("predictions_file", ""))
...
def select(run_dir: Path, validation_root: Path) -> dict[str, Any]:
    candidates = []
    for step in (500, 1000, 1500):
        ...
        candidates.append(validate_candidate(metrics, expected_step=step, expected_model=model))
...
def main() -> None:
    ...
    try:
        result = select(args.run_dir, args.validation_root)
