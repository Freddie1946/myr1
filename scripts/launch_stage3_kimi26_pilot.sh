...
RUN_DIR="${PATHVLM_STAGE3_RUN_DIR:-$INSTALL_ROOT/runs/stage3_process_grpo/kimi26_50step_seed42_20260730}"
...
REWARD_LOG_DIR="$RUN_DIR/reward_audit_attempt03"
SMOKE_MARKER="$RUN_DIR/kimi26_smoke_v2_passed.json"
TRAIN_LOG="$RUN_DIR/train_attempt03.log"
...
if [[ ! -f "$SMOKE_MARKER" ]]; then
  echo "Kimi 2.6 paid smoke marker is missing: $SMOKE_MARKER" >&2
  exit 2
fi
...
"$PYTHON" - "$DATASET" "$IMAGE_HASH_MANIFEST" "$SMOKE_MARKER" "$MASTER_PORT" <<'PY'
import hashlib
import json
import socket
import sys
...
if mismatch:
    raise SystemExit(f"Kimi smoke marker mismatch: {mismatch}")
...
print("Stage3 static preflight passed")
PY
...
export PATHVLM_TRAINING_SEGMENT="kimi26_stage3_pilot_50step_seed42_attempt03"
...
export PATHVLM_OPENROUTER_MAX_JUDGE_TOKENS="640"
...
export PATHVLM_OPENROUTER_MAX_UNIQUE_REQUESTS="411"
export PATHVLM_TRAIN_STATE_AUDIT_NAME="kimi26_stage3_train_state_audit.json"
...
printf 'Starting Kimi 2.6 Stage3 pilot; log=%s\n' "$TRAIN_LOG"
