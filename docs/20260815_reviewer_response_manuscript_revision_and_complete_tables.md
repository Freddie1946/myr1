...
为降低专家逐题浏览成本，材料另提供 reference-assisted 入口：每题直接并列展示匿名病例内容与外部意见，并有上一题/下一题导航。奖励页已严格收缩为训练 Judge 的六个布尔事件，每项可直接点击 True/False，并在其下显示 Claude Sonnet 4.6 的对应布尔参考；不再要求任何 1–5 分或补充质量字段。应用服务器将每题评分原子写入 JSON 和逐评审者 CSV，并即时计算人工与外部参考在同一 0.4 公式下的最终过程分差。该入口仍对候选模型身份盲化，但不对外部参考盲化，因此其评分只作为辅助/仲裁结果，不能取代上述独立盲评主统计。

该辅助入口现以中英文对照展示评分规则、题干、选项、参考答案、匿名待评回答、外部模型理由和 ROI 特征；英文原文继续保留为权威文本，翻译只用于辅助理解。冻结 60 例上，Claude Sonnet 4.6 与训练期 GPT-4o 的换算过程奖励精确同分率为 38.33%，85.00% 的病例差值不超过 0.2，平均绝对分差为 0.1617；六事件 micro agreement 79.72% 和 Cohen's kappa 0.566 仅作为分歧来源的辅助诊断。这属于中等机器—机器接近程度，不能替代尚待完成的病理专家复核；challenge-enriched 面板也不能当作总体随机样本。人工完成后，主要报告同一 0.4 公式下的人类−GPT-4o逐病例最终分差、MAE、RMSE、精确同分率、阈值内比例及 bootstrap CI，而不是把事件总体一致率作为主终点。
