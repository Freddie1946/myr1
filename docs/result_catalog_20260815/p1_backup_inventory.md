...
P1 使用 `public + manual gated` 仓库存放只含可加载权重的关键快照；不含 optimizer、scheduler 和 RNG state。总选中文件约 183.32 GB（170.73 GiB）。
...
| Full rule-RL n4 | step1000 | 16.60 | `PathVLM-R1-FullRuleRL-n4-step1000-seed42-GatedArchive` | 正在上传；与现有 private n4 四个权重分片 SHA256 全部相同，是精确重复迁移 |
...
| Grok-4.3 Stage3 | step500/1000/1500 | 49.80 | `PathVLM-R1-Stage3-Grok43-seed42-GatedArchive` | private Grok 权重与本集合的 step1500 四个分片全部相同；500/1000 不同，属于部分精确重叠 |
| Historical outcome-GRPO | epoch2/step1000 | 16.60 | `PathVLM-R1-Outcome-GRPO-n1000-seed42-epoch2-GatedArchive` | 与现有 private Outcome-GRPO 四个分片全部相同，是精确重复迁移 |
| Historical full-SFT4000 control | checkpoint250 | 16.60 | `PathVLM-R1-Full-SFT4000-Control-seed42-GatedArchive` | 与现有 private SFT4000 四个分片全部相同，是精确重复迁移 |
